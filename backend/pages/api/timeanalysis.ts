...
export default function handler(
  ...
  if (typeof sprint === 'string' && sprint.length > 0) {
    data = buildTimeAnalysis(sprint, dataMode, projectName, undefined, undefined, filterType)
  } else if (typeof startDate === 'string' && typeof endDate === 'string' && startDate && endDate) {
    data = buildTimeAnalysis(null, dataMode, projectName, startDate, endDate, filterType)
  } else {
